...
type AdminPanelProps = {
  ...
  stockActiveByProductId: Record<string, boolean>;
  setStockActiveByProductId: (updater: (current: Record<string, boolean>) => Record<string, boolean>) => void;
  ...
  toggleStockActive: (productId: string) => void | Promise<void>;
          ...
          {adminTab === "transactions" && (
            <>
              ...
              <div className="grid gap-3 md:grid-cols-3 lg:grid-cols-7">
                <label className="flex flex-col gap-2 text-sm"><span className="text-xs uppercase tracking-wide text-slate-500">Status</span><select value={adminStatusFilter} onChange={(event) => onAdminStatusFilterChange(event.target.value as "all" | TransactionStatus)} className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-700 outline-none transition focus:border-slate-500 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-200"><option value="all">All</option><option value="pending">Pending</option><option value="completed">Completed</option><option value="canceled">Canceled</option><option value="abandoned">Abandoned</option></select></label>
